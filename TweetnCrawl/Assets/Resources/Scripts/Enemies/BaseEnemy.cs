...
public class BaseEnemy : MonoBehaviour {
    ...
    public IEnumerator OnDeathEffect()
    {
        if (!dead)
        {
            ...
            Physics2D.IgnoreCollision(explosion.GetComponent<Collider2D>(), gameObject.GetComponent<Collider2D>());
            Destroy(gameObject);
	...
	public  IEnumerator ShowMessage() {
		...
		//The boolean is VERY important if it's removed it will cause duplicates of the message
			if(message == 1 && isCreated == false) {
				...
				myText.GetComponent<Renderer>().sharedMaterial = ArialFont.material;
				myText.fontSize = 10;
	...
	protected virtual IEnumerator RandomPlayerSpottedSound() {
		if (distance < chaseRange - 25 && !GetComponent<AudioSource>().isPlaying) {
			int wait = Random.Range(3,15);
			yield return new WaitForSeconds(wait);
			GetComponent<AudioSource>().PlayOneShot (PlayerSpottedSounds [Random.Range (0, PlayerSpottedSounds.Length)]);
		}
		yield return null;
